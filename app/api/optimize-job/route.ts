...
// ABOUTME: Server-side API route for AI-powered job optimization using shared Gemini client
...
Optimize the requested aspects and return JSON.`
    ...
    // Call shared Gemini client with retry logic (uses gemini-3-flash-preview)
    try {
      const text = await geminiClient.processRow(fullPrompt, {})
