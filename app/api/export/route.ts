/**
 * ABOUTME: API Route for export (localStorage-based auth)
 * ABOUTME: Exports results to CSV/JSON - no database batch verification
 */
...
export async function POST(request: NextRequest): Promise<Response> {
  try {
    ...
    // Generate user-oriented filename (no database lookup in local mode)
    const date = timestamp ? new Date(timestamp) : new Date()
    const filename = generateExportFilename(null, date, format)
