...
describe('useBatchProcessor', () => {
  ...
  describe('startBatch', () => {
    const mockCSVData: ParsedCSV = {
      ...
      rows: [
        { rowIndex: 0, data: { name: 'Alice', company: 'Acme' } },
        { rowIndex: 1, data: { name: 'Bob', company: 'Tech' } },
      ],
      totalRows: 2,
    }
      ...
      await act(async () => {
        await result.current.startBatch({
          csvData: {
            filename: 'test.csv',
            columns: ['name'],
            rows: [{ rowIndex: 0, data: { name: 'Test' } }],
            totalRows: 1,
          },
          prompt: 'Test',
          outputColumns: ['bio'],
      ...
      await act(async () => {
        await result.current.startBatch({
          csvData: {
            ...
            columns: ['name'],
            rows: [
              { rowIndex: 0, data: { name: 'User1' } },
              { rowIndex: 1, data: { name: 'User2' } },
            ],
            totalRows: 2,
          },
          prompt: 'Test',
