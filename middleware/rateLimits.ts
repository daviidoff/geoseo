/**
 * ABOUTME: Rate limiting configuration and middleware (localStorage-based)
 * ABOUTME: In localStorage mode, rate limits are minimal (no database tracking)
 *
 * Rate limiting configuration and middleware
 * In localStorage mode, uses in-memory tracking (resets on server restart)
 */
...
const userBatchCounts = new Map<string, { currentBatches: number; totalBatchesToday: number }>()

/**
 * Get current batch counts (in-memory for localStorage mode)
 */
async function getUserBatchCounts(userId: string): Promise<{
  currentBatches: number
  totalBatchesToday: number
}> {
  // In localStorage mode, use in-memory tracking
  const counts = userBatchCounts.get(userId) || { currentBatches: 0, totalBatchesToday: 0 }
  return counts
}
