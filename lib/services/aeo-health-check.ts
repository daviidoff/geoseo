...
// Helper functions for scoring (extracted from nested ternaries)
function scoreLengthRange(
  value: string | undefined,
  min: number,
  max: number,
  fieldName: string
): { status: 'pass' | 'warning' | 'fail'; score: number; recommendation?: string } {
  if (!value) {
    return { status: 'fail', score: 0, recommendation: `Add a ${fieldName}` }
  }
  if (value.length >= min && value.length <= max) {
    return { status: 'pass', score: 100 }
  }
  if (value.length < min) {
    return { status: 'warning', score: 70, recommendation: `${fieldName} too short (aim for ${min}-${max} chars)` }
  }
  return { status: 'warning', score: 70, recommendation: `${fieldName} too long (aim for ${min}-${max} chars)` }
}

function scoreExactCount(
  value: string | undefined,
  count: number,
  expected: number,
  fieldName: string
  ...
  if (!value) {
    return { status: 'fail', score: 0, recommendation: `Add ${fieldName.startsWith('a') ? 'an' : 'a'} ${fieldName}` }
  }
  if (count === expected) {
    return { status: 'pass', score: 100 }
  }
  return { status: 'warning', score: 70, recommendation: `Use only one ${fieldName} per page` }
}
...
function runTechnicalSEOChecks($: cheerio.CheerioAPI, url: string, headers: Headers): HealthCheck[] {
  ...
  const titleScore = scoreLengthRange(title || undefined, 30, 60, 'title tag')
  checks.push({
    name: 'Title Tag',
  ...
  const metaDescScore = scoreLengthRange(metaDesc || undefined, 120, 160, 'meta description')
  checks.push({
  ...
  const h1Score = scoreExactCount(h1 || undefined, h1Count, 1, 'H1 tag')
  checks.push({
    name: 'H1 Tag',
    category: 'Technical SEO',
    ...h1Score,
