 ...
 * @deprecated Use createUsageLimitResponse instead
 */
export function createInsufficientCreditsResponse(
  required: number,
  available: number
): NextResponse<APIErrorResponse> {
  return createUsageLimitResponse(available, required)
}
...
export function createUsageLimitResponse(
  ...
  return createErrorResponse(
    new BulkGPTError(
      'RATE_LIMIT_EXCEEDED',
      `Usage limit reached. Used: ${used}, Limit: ${limit}. Upgrade for more.`,
      { used, limit, upgrade: true }
    ),
...
export function createServiceUnavailableResponse(
  ...
  return createErrorResponse(
    new Error(service ? `${service} is temporarily unavailable` : 'Service temporarily unavailable'),
    503
  )
}
