...
/**
 * Responsive variant interface for mobile/tablet/desktop classes
 */
...
export function getResponsiveClasses<T extends Record<string, unknown>>(
  ...
  for (const [key, variants] of Object.entries(utilities)) {
    if (variants && typeof variants === 'object') {
      const typedVariants = variants as ResponsiveVariant
      if (state.isMobile) {
        result[key as keyof T] = typedVariants.mobile || typedVariants.tablet || typedVariants.desktop || ''
      } else if (state.isTablet) {
        result[key as keyof T] = typedVariants.tablet || typedVariants.desktop || typedVariants.mobile || ''
      } else {
        result[key as keyof T] = typedVariants.desktop || typedVariants.tablet || typedVariants.mobile || ''
      }
    }
  }
