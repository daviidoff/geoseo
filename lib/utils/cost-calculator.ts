...
export function getCostSavingsRecommendation(
  ...
  if (normalizedModel.includes('pro')) {
    const flashCost = calculateCost('gemini-3-flash-preview', tokens)
...
export function calculatePotentialSavings(
  ...
  for (const [model, stats] of Object.entries(modelBreakdown)) {
    ...
    const modelFlashCost = calculateCost('gemini-3-flash-preview', { input: stats.input, output: stats.output })
