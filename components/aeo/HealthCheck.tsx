...
import { generateHealthExcel, downloadBlob } from '@/lib/exports/aeo-excel-generator'
...
export function HealthCheck() {
  ...
  const handleCheck = async () => {
    ...
    try {
      ...
      if (data.checks) {
        const categories = Array.from(new Set(data.checks.map((c: any) => c.category))) as string[]
  ...
  const handleExportPDF = async () => {
    ...
    try {
      const blob = await generateHealthReportPdf(result, result.url)
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const blob = await generateHealthExcel(result, result.url)
